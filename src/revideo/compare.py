...
def compare(original: str, replica: str) -> dict:
    ...
    rhythm = np.mean([x for x in (_ratio_score(fa["shot_count"], fb["shot_count"]),
                                  _ratio_score(fa["asl_sec"], fb["asl_sec"]), shape) if x is not None])
    parts["rhythm"] = {"score": float(rhythm), "shots": [fa["shot_count"], fb["shot_count"]],
                       "asl": [fa["asl_sec"], fb["asl_sec"]], "cut_timing_shape": round(shape, 3)}
    ...
    parts["color"] = {"score": float(np.mean(col)) if col else None, "palette_delta_e": dE,
                      "look_original": ga.get("look_labels"), "look_replica": gb.get("look_labels")}
    ...
    parts["framing"] = {"score": float(np.mean([x for x in (ar, overlap) if x is not None])),
    ...
    parts["camera"] = {"score": sum(min(ca.get(k, 0) / ta, cb.get(k, 0) / tb) for k in keys) if keys else None,
                       "original": ca, "replica": cb}

    # sound: tempo + cut sync
    snd = [x for x in (_ratio_score(fa.get("tempo_bpm_estimate"), fb.get("tempo_bpm_estimate")),
                       None if fa.get("cuts_on_beat_ratio") is None or fb.get("cuts_on_beat_ratio") is None
                       else 1 - abs(fa["cuts_on_beat_ratio"] - fb["cuts_on_beat_ratio"])) if x is not None]
    parts["sound"] = {"score": float(np.mean(snd)) if snd else None,
    ...
    return {"fidelity_score": round(total * 100, 1), "weights": WEIGHTS, "components": parts,
